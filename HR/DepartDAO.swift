...
class DepartDAO{
    //튜플(파이썬, 스위프트, 코틀린에 있는 자료형으로 서로 다른 자료형으로 묶어서 저장할 수 있는 자료형)에 별명 만들기
    //스위프트나 파이썬에서는 VO 클래스 대신에 튜플을 사용하는 것을 권장하고 데이터 분석분야의 많은 메소드들이 튜플을 리턴한다.
    typealias DepartRecord = (Int, String, String)
    
    //lazy init - 지연생성
    //참조형 변수의 데이터를 처음에 만들어두지 않고 최초로 사용을 할 때 만드는 기법
    //이 기법을 적절히 이용하면 적은 메모리를 가지고 프로그램을 실행할 수 있다.
